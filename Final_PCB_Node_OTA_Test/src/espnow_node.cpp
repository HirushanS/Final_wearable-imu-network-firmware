...
bool sendImuFrameNow(IMU_Node_Frame &frame) {
  ...
  frame.payloadType = IMU_PAYLOAD_TYPE_COMPACT_INT16;

  CompactIMUFrame compactFrame;
  encodeCompactIMUFrame(frame, compactFrame);

#if IMU_PREPROCESSING_TEST_MODE && IMU_TEST_DISABLE_ESPNOW_IMU_SENDS
  sequenceNumber++;
  return true;
#endif

  if (!sendEspNowPacket(
        reinterpret_cast<const uint8_t *>(&compactFrame),
        sizeof(compactFrame)
      )) {
    return false;
  }
