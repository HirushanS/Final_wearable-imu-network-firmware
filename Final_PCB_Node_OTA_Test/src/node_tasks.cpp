...
void bno085AcquisitionTask(void *parameter) {
  ...
  clearImuFrame(
    sampledFrame,
    NODE_ID,
    IMU_PAYLOAD_TYPE_COMPACT_INT16
  );

  IMUPreprocessorConfig preprocessorConfig;
  IMUPreprocessorState preprocessorState;
  uint16_t testSequenceNumber = 0;
  ...
  while (true) {
    ...
    if (
      ...
      IMU_Node_Frame rawFrame = sampledFrame;
      sampledFrame.timestamp_ms = getNodeTimestampMs();
      sampledFrame.sequenceNo = testSequenceNumber++;
      ...
      CompactIMUFrame compactFrame;
      encodeCompactIMUFrame(sampledFrame, compactFrame);
